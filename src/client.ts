export const client = String.raw`(() => {
  ...
  const copy = {
    inspect:"Inspect", toolbar:"OpenCode visual inspector", noAnnotations:"No annotations", annotation:"annotation", annotations:"annotations", clear:"Clear", send:"Send to OpenCode", exit:"Exit inspector",
    ...
    sending:"Sending...", retry:"Retry", chooseDelivery:"Send annotations", chooseDeliveryBody:"Who should implement this batch?",
  ...
  const css = function(){/*
    ...
    *{box-sizing:border-box}
    ...
    @media(prefers-reduced-motion:reduce){.bar,.dialog,.box,.scrim,button{transition-duration:.01ms!important;transform:none}.bar{transform:translateX(-50%)}.bar.open{transform:translateX(-50%)}.dialog{filter:none}.decision,.decision.open{transform:translate(-50%,-50%)}}
    ...
    @media(max-width:720px){.bar{bottom:10px;max-width:calc(100vw - 16px)}.brand>span{display:none}.brand svg{width:18px;height:18px}.rule{display:none}.clear{display:none}.view-label{max-width:76px;overflow:hidden;text-overflow:ellipsis}}
  ...
  */}.toString();
  ...
  const resize = (item) => {
    active = item;
    remember(item);
    viewLabel.textContent = item.width + "×" + item.height;
    window.resizeTo(item.width + Math.max(0, outerWidth - innerWidth), item.height + Math.max(0, outerHeight - innerHeight));
    close(true);
  };
  const sizes = () => {
    close(true);
    ...
    dialog.querySelector(".apply").onclick = () => {
      ...
      resize({ label:t("custom"), width, height });
    };
    root.append(dialog);
  ...
  const inspect = (el, x, y) => {
    ...
    dialog.innerHTML = '<div class="title" id="oc-dialog-title" data-copy="describe"></div><div class="meta"></div><span class="target-label" data-copy="target"></span><div class="targets"></div><label for="oc-change" data-copy="change"></label><textarea id="oc-change"></textarea><div class="actions"><button class="cancel secondary" type="button" data-copy="cancel"></button><button class="add primary" type="button" data-copy="add"></button></div><div class="error" role="alert" hidden></div>';
    fill(dialog);
    dialog.querySelector("textarea").placeholder = t("placeholder");
  ...
  view.onclick = sizes;
  clear.onclick = () => { notes.length = 0; views.length = 0; refresh(); };
  ...
  const done = () => {
    ...
    choice(node, t("closeWindow"), "", () => { window.close(); setTimeout(() => exit.click(), 100); });
